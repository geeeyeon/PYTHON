...
def photo_add() :
    ...
    outH = inH
    value=0
    value = askinteger('밝게(곱셈)','값을 입력하세요',minvalue=1, maxvalue=255)
    #출력 영상 메모리 확보
    outImage = []
    for i in range(0,inW) :
        tmpList = []
...
def photo_sub() :
    ...
    outH = inH
    value=0
    value = askinteger('어둡게(나눗셈)','값을 입력하세요',minvalue=1, maxvalue=255)
    #출력 영상 메모리 확보
    outImage = []
    for i in range(0,inW) :
        tmpList = []
...
photo1Menu.add_command(label='밝게하기(곱셈)', command=photo_add)
photo1Menu.add_command(label='어둡게하기(나눗셈)', command=photo_sub)
photo1Menu.add_command(label='상하반전', command=photo_reverse1)
photo1Menu.add_command(label='좌우반전', command=photo_reverse2)
