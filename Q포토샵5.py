...
def photo_r():
    global inImage, outImage, inH, inW, outH, outW, window, canvas, paper, filename
    value = askfloat('감마', '감마 값 입력(0~2)', minvalue = 0, maxvalue = 2)
    # 출력 파일의 크기 결정
    outW = inW
    outH = inH
    # 출력 영상 메모리 확보
...
def photo_translate():
    global inImage, outImage, inH, inW, outH, outW, window, canvas, paper, filename
    Xvalue = askinteger('X위치', 'X위치 입력(0~' + str(inW) + ')' , minvalue = 0, maxvalue = inW)
    Yvalue = askinteger('Y위치', 'Y위치 입력(0~' + str(inH) + ')', minvalue = 0, maxvalue = inH)
     # 출력 파일의 크기 결정
    outW = inW
    outH = inH
    # 출력 영상 메모리 확보
    ...
    for i in range(0, outW):
        ...
        outImage.append(tmpList)
    ### 진짜 영상 처리 알고리즘!!! ###
    for i in range(0, outW - Xvalue):
        for k in range(0, outH - Yvalue):
            outImage[i + Xvalue][k + Yvalue] = inImage[i][k]
    displayImage(outImage, outW, outH, inW)
...
photo3Menu.add_command(label='감마', command=photo_r)
photo3Menu.add_command(label='이동', command=photo_translate)
